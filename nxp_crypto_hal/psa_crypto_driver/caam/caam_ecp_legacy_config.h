 ...
 * out by TF-PSA-Crypto unless opted back in.  It is injected via
 * -include in the CAAM PSA crypto driver CMake target so that both the
 * driver sources and the builtin library see the same defines.
 */

#ifndef CAAM_ECP_LEGACY_CONFIG_H
#define CAAM_ECP_LEGACY_CONFIG_H
...
/* Legacy mbedTLS ECP/bignum modules required by the CAAM key-management port. */
...
/* Enable SECP-R1 curves used by the CAAM driver. */
...
/* Ensure matching PSA_WANT_ECC_SECP_R1_* macros are set so that
 * psa_util_internal.c's mbedtls_ecc_group_from_psa() returns the correct
 * group IDs for CAAM-accelerated curves. */
 ...
 * key-pair or ECDSA algorithm is requested.
 */
...
/*
 * ECDSA signing uses mbedtls_hmac_drbg_* and mbedtls_md_info_from_type(),
 * which require MBEDTLS_MD_C and MBEDTLS_HMAC_DRBG_C to be compiled in.
 */
...
#endif /* CAAM_ECP_LEGACY_CONFIG_H */
