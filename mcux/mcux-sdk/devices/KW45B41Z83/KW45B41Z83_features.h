...
/* @brief Is affected by errata with ID 5829 (FlexCAN: FlexCAN does not transmit a message that is
 * enabled to be transmitted in a specific moment during the arbitration process).
 */
 ...
 * transmitted into the CAN bus when the Message Buffer under transmission is either aborted or
 * deactivated while the CAN bus is in the Bus Idle state).
 */
#define FSL_FEATURE_FLEXCAN_HAS_ERRATA_6032 (0)
/* @brief Is affected by errata with ID 9595 (FlexCAN: Corrupt frame possible if the Freeze Mode or
 * the Low-Power Mode are entered during a Bus-Off state).
 */
...
/* @brief Number of DMA channels (related to number of registers TCD, DCHPRI, bit fields ERQ[ERQn],
 * EEI[EEIn], INT[INTn], ERR[ERRn], HRS[HRSn] and bit field widths ES[ERRCHN], CEEI[CEEI],
 * SEEI[SEEI], CERQ[CERQ], SERQ[SERQ], CDNE[CDNE], SSRT[SSRT], CERR[CERR], CINT[CINT],
 * TCDn_CITER_ELINKYES[LINKCH], TCDn_CSR[MAJORLINKCH], TCDn_BITER_ELINKYES[LINKCH]).
 * (Valid only for eDMA modules.)
 */
#define FSL_FEATURE_EDMA_MODULE_CHANNEL (16)
/* @brief Number of DMA channel groups (register bit fields CR[ERGA], CR[GRPnPRI], ES[GPE],
 * DCHPRIn[GRPPRI]). (Valid only for eDMA modules.)
 */
...
/* @brief Number of channel for each EDMA instance, (only defined for soc with different channel
 * numbers for difference instance)
 */
...
/* @brief Capacity (number of entries) of the transmit/receive FIFO (or 0 if no FIFO is available)*/
...
/* @brief Has low power features (can be enabled in wait mode via register bit C1[DOZEEN] or
 * CTRL[DOZEEN] if the registers are 32-bit wide).
 */
#define FSL_FEATURE_LPUART_HAS_LOW_POWER_UART_SUPPORT (1)
/* @brief Has extended data register ED (or extra flags in the DATA register if the registers
 * are 32-bit wide).
 */
 ...
 * or BAUD[OSR], BAUD[BOTHEDGE], BAUD[RESYNCDIS] if the registers are 32-bit wide).
 */
...
/* @brief Has transmitter/receiver DMA enable bits C5[TDMAE]/C5[RDMAE] (or BAUD[TDMAE]/BAUD[RDMAE]
...
/* @brief Data character bit order selection is supported (bit field S2[MSBF] or STAT[MSBF] if the
...
#define FSL_FEATURE_TPM_CHANNEL_COUNTn(x) \
	...
	(((x) == TPM1) ? (6) : \
	(((x) == TPM2) ? (2) : (-1))))
/* @brief Has counter reset by the selected input capture event
 * (register bits C0SC[ICRST], C1SC[ICRST], ...).
 */
