...
"""
Wrapper around lpc_cfg_tools.py to provide arguments
"""
...
import lpc_cfg_utils

HELPSTR = """
Processes NXP LPC signal configuration files and outputs lpc pinctrl
headers. Only supports lpc parts.
...
parser.add_argument('processor_root', metavar = 'PROC_ROOT', type=str,
...
if not args.soc_output:
    ...
    args.soc_output = args.soc_output.rstrip('/')
if not pathlib.Path(args.soc_output).is_dir():
...
for directory in proc_root.iterdir():
    ...
    if signal_path.exists():
        if args.copyright:
            ...
            copyright_txt = (f"Copyright {datetime.datetime.today().year} NXP\n"
            f" * SPDX-License-Identifier: Apache-2.0")
            util = lpc_cfg_utils.NXPSdkUtil(str(signal_path), copyright_header= copyright_txt)
        else:
            util = lpc_cfg_utils.NXPSdkUtil(str(signal_path))
