...
common_device_files = [
    'fsl_dcdc.c', 'fsl_dcdc.h',
    'fsl_nic301.h',
    'fsl_flexram_allocate.c', 'fsl_flexram_allocate.h',
...
def import_sdk(directory, device_drivers_pattern = ""):
    ...
    for device in devices:
        ...
        # Some files are modified differently between devices. Put them
        # in the device specific folder.
        common_tmp = []
        for i in range(len(shared_drivers) - 1, -1, -1):
            tfile = os.path.basename(shared_drivers[i])
            if tfile in common_device_files:
               common_tmp.append(shared_drivers[i])
        ...
        # There are some device files that appear to be unique per device
        # so move them to the device_dst instead of shared_dst.
        print('Importing {} xip boot to {}'.format(device, device_dst))
        #print('   xip_boot_src: {},\n   xip_boot_pattern: {},\n   device_dst: {}'.format(xip_boot_src, xip_boot_pattern, device_dst))
        copy_files(xip_boot, device_dst)
