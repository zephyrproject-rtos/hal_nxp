...
HELPSTR = """
Processes NXP LPC MEX configuration files
Given a mex file, generates the board level pinctrl groups required for
Zephyr. Only supports lpc parts.
...
board_name = lpc_cfg_utils.get_board_name(args.mex_file)
processor_name = lpc_cfg_utils.get_processor_name(args.mex_file)
package_name = lpc_cfg_utils.get_package_name(args.mex_file)
...
if args.copyright:
    ...
    util = lpc_cfg_utils.NXPSdkUtil(signal_file, copyright_header= copyright_txt)
else:
    util = lpc_cfg_utils.NXPSdkUtil(signal_file)
