...
void wlan_cancel_host_sleep(void)
{
    ...
    if (!wlan_is_started())
    {
        wlcm_w("Wlan not started, can't cancel host sleep");
        return;
    }
...
#define MAX_CHANNEL_BAND_B 14
            ...
            if(is_uap_started())
            {
                ...
                (void)wlcm_d("uap switch to channel %d success!", channel);
            }
#endif
            ...
            if (is_sta_connected())
            {
                ...
                (void)wlcm_d("sta switch to channel %d success!", channel);
            }
#if !CONFIG_MEM_POOLS
            OSA_MemoryFree((void *)msg->data);
#else
...
static void wifi_process_bg_scan_stopped(struct wifi_message *msg)
{
    ...
    else
    {
        wlan.bgscan_attempt = 0;
        wlan.roam_reassoc = false;
        wlcm_d("Soft Roam: AP with better RSSI not found");
...
static void temperature_mon_cb(osa_timer_arg_t arg)
{
    ...
    {
        struct wlan_message msg;
        wlcm_d("recovery_enable: %u, wifi_fw_is_hang: %u, reset_in_progress:%u",
