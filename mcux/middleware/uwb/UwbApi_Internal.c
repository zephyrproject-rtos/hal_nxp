...
tUWBAPI_STATUS recoverUWBS(void)
{
    ...
    uwbContext.dev_state = kUci_DeviceState_NA;
    status = (uint8_t)UwbDeviceInit(TRUE);
    LOG_DBG("%s: uwb device init status: %d", __FUNCTION__, status);
    if (status == UWBAPI_STATUS_OK) {
        status = setDefaultCoreConfigs();
        if (status != UWBAPI_STATUS_OK) {
            LOG_ERR("%s: setDefaultCoreConfigs is failed:", __FUNCTION__);
            return status;
        }

        // Update UWBC device info
        phUwbDevInfo_t dev_info = {0};
        UwbApi_GetDeviceInfo(&dev_info);
        // status = getDeviceInfo();
    }
    else {
        LOG_ERR("%s: DownloadFirmware is failed:", __FUNCTION__);
        return status;
    }
    return status;
}
...
int uwbInit()
{
    ...
    if (k_sem_init(&uwbContext.uwb_binding_status_ntf_wait, 0, 1) != kUwb_StatusCode_Success) {
    ...
    if (uwbContext.fwMode == MAINLINE_FW) {
        ...
        if (k_sem_take(&
                uwbContext.uwb_binding_status_ntf_wait, Z_TIMEOUT_MS(UWBD_BINDING_STATUS_NTF_TIMEOUT)) == kUwb_StatusCode_Success) {
...
tUWBAPI_STATUS sendRawUci2(uint8_t *p_cmd_params, uint16_t cmd_params_len, uint8_t *response, uint32_t *response_len)
{
    ...
    if (header->mt == UCI_MT_DATA) {
        ...
        ret = uwb_uci_transceive_data_packet(header->gid,
                        p_cmd_params + UCI_HEADER_SIZE, cmd_params_len - UCI_HEADER_SIZE,
                        response, response_len);
    } else {
        /* Control packet: route through control transceive path */
        ret = uwb_uci_transceive_control_packet(header->gid, header->oid,
                        p_cmd_params + UCI_HEADER_SIZE, cmd_params_len - UCI_HEADER_SIZE,
                        response, response_len);
    }
...
tUWBAPI_STATUS getDeviceInfo(phUwbDevInfo_t *pdevInfo)
{
    ...
    if (kUwb_StatusCode_Success != status_code) {
        /** Failed */
        return UWBAPI_STATUS_FAILED;
    }
