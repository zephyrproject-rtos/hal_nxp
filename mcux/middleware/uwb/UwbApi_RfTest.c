...
EXTERNC tUWBAPI_STATUS UwbApi_SetRfTestParams(uint32_t sessionHandle, const phRfTestParams_t *pRfTestParams)
{
    ...
    void *scheduled_ntf = uwb_uci_schedule_packet_read(
        UCI_MT_NTF, UCI_GID_SESSION_MANAGE, UCI_MSG_SESSION_STATUS_NTF, ese_session_id_list_ntf, &ese_session_id_list_ntf_len, &g_uwb_nxp_semaphore);
    ...
    if (kUwb_StatusCode_Success != k_sem_take(&g_uwb_nxp_semaphore, timeout)) {
        LOG_ERR("Did not receive notification");
        status = kUwb_StatusCode_Failed;
    }
    else {
        UWB_STREAM_TO_UINT32(notified_session_handle, ese_session_id_list_ntf, offset);
        UWB_STREAM_TO_UINT8(notified_session_state, ese_session_id_list_ntf, offset);
        if (notified_session_handle != sessionHandle) {
            LOG_ERR("Received unknown notification");
            status = kUwb_StatusCode_Failed;
        }
    }

    if ((status == UWBAPI_STATUS_OK) && (uwbContext.sessionInfo.state != kUwb_SessionStatus_Idle)) {
        // status = waitforNotification(UWA_DM_SESSION_STATUS_NTF_EVT, timeout);
        if (notified_session_handle != sessionHandle || notified_session_state != kUwb_SessionStatus_Idle) {
...
EXTERNC tUWBAPI_STATUS UwbApi_Stop_RfTest(void)
{
    ...
    if (status == UWBAPI_STATUS_OK) {
        ...
        if (uwbContext.sessionInfo.state != kUwb_SessionStatus_Idle) {
            status = UWBAPI_STATUS_FAILED;
        }
        uint8_t response[10] = {0};
        uint32_t response_len = sizeof(response);
        int ret = uwb_uci_transceive_control_packet(UCI_GID_SESSION_MANAGE, UCI_MSG_TEST_STOP_SESSION,
