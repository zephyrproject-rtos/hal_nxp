...
EXTERNC tUWBAPI_STATUS UwbApi_SetAppConfigWrappedRDS(uint32_t sessionHandle, uint8_t *pWrappedRds, size_t WrappedRdsLen)
{
    ...
    if (pWrappedRds == NULL) {
        ...
        status = UWB_STATUS_CODE_INVALID_ARGUMENT;
        goto exit;
    }

    cmdLen                        = uwb_serialize_app_config_payload(sessionHandle, 1, payloadOffset, uwbContext.snd_data);
    if ((cmdLen + 2 + WrappedRdsLen) > sizeof(uwbContext.snd_data)) {
        LOG_ERR("%s: Wrapped RDS data exceeds buffer size (cmdLen=%u, WrappedRdsLen=%zu, max=%zu)",
            __FUNCTION__, cmdLen, WrappedRdsLen, sizeof(uwbContext.snd_data));
        status = UWB_STATUS_CODE_INSUFFICIENT_BUFFER;
        goto exit;
    }
    ...
    const UWB_DebugParams_List_t *DebugParams_List)
{
    ...
    status = UwbApi_SessionInit(UwbPhoneConfig.session_id, UWB_SESSION_TYPE_RANGING, &sessionHandle);
    if (status != UWBAPI_STATUS_OK) {
