...
{
    tUWBAPI_STATUS status                 = UWB_STATUS_CODE_INVALID_ARGUMENT;
...
{
    LOG_DBG("%s: enter", __FUNCTION__);
    if (UWB_STATUS_CODE_SUCCESS == uwb_api_core_device_reset(resetConfig)) {
        return UWBAPI_STATUS_OK;
    }
    return UWBAPI_STATUS_FAILED;
}
...
EXTERNC tUWBAPI_STATUS UwbApi_GetUwbDevState(uint8_t *pDeviceState)
{
    ...
    uwb_config_t config = {
        .tag = UWB_CORE_CONFIG_DEVICE_STATE,
        .length = 1,
        .value = pDeviceState,
        .status = 0,
    };
    ...
    if ((UWB_STATUS_CODE_SUCCESS != uwb_status) || (0 != config.status)) {
        return uwb_status;
    }
...
EXTERNC tUWBAPI_STATUS UwbApi_SetRangingParams(uint32_t sessionHandle, const phRangingParams_t *pRangingParam)
{
    ...
    if (UWB_SESSION_STATUS_INITIALIZED == session_state) {
    ...
    if (UWB_SESSION_STATUS_INITIALIZED == session_state) {
        do {
            ...
            if (UWB_STATUS_CODE_SUCCESS != k_sem_take(&g_uwb_nxp_semaphore, uwb_ntf_timeout)) {
                ...
                if (ntf_session_handle != sessionHandle) {
                    ...
                    status = UWB_STATUS_CODE_FAILED;
                    continue;
                } else if (UWB_SESSION_STATUS_IDLE != retrieved_session_state) {
    ...
    uint32_t sessionHandle, uint8_t noOfparams, const UWB_VendorAppParams_List_t *vendorAppParams_List)
{
    ...
    for (uint32_t LoopCnt = 0; LoopCnt < noOfparams; ++LoopCnt) {
        ...
        {
            uint16_t param_tlv_len = getVendorAppConfigTLVBuffer(paramId,
                                                 pParamValue,
                                                 singleParamLen,
                                                 NULL);
            if (param_tlv_len + paramLen <= sizeof(uwbContext.snd_data)) {
    ...
    phMulticastControleeListRspContext_t *pControleeListRspContext)
{
    ...
    if (pControleeContext->action == MULTICAST_LIST_DEL_CONTROLEE) {
        for (uint8_t i = 0; i < pControleeContext->no_of_controlees; i++) {
            if (pControleeContext->controlee_list[i].subsession_id != 0x0) {
                return UWB_STATUS_CODE_INVALID_ARGUMENT; // for deletion, sub Session Handle must be zero
            }
        }
    }
    ...
    for (uint8_t controlee_index = 0; controlee_index < pControleeContext->no_of_controlees; controlee_index++) {
        if (UCI_STATUS_CODE_OK != controlee_list[controlee_index].status) {
#if UWBIOT_UWBD_SR1XXT_SR2XXT
    ...
    if (uwb_status & UWB_COMPONENT_STATUS_BITMASK){
        return uwb_status;
    }
    pControleeListRspContext->status = (uint8_t)uwb_status;
    ...
    UWB_DeviceRole_t device_role, AccessoryUwbConfigDataContent_t *uwb_data_content)
{
    ...
    sys_put_le16((readMMId), &uwb_data_content->model_id[(2)]);
    if (status != UWBAPI_STATUS_OK) {
...
tUWBAPI_STATUS UwbApi_GetUwbConfigData_Android(UwbDeviceConfigData_t *uwb_data_content)
{
    ...
    if(uwb_data_content->isFindhubEnabled  == false){
        ...
        status = UwbApi_SessionInit(0x11223344, UWB_SESSION_TYPE_RANGING, &sessionHandle);
        if (status != UWBAPI_STATUS_OK) {
