...
enum _lpsci_status_flag_t
{
    ...
    kLPSCI_ParityErrorInRxDataRegFlag =
        (UART0_ED_PARITYE_MASK << 16), /*!< PARITY bit, sets if noise detected in current data word */
#endif
};
...
typedef void (*lpsci_isr_t)(UART0_Type *base, void *handle);
...
extern const IRQn_Type s_lpsciIRQ[];

/*! Pointers to lpsci handles for each instance. */
extern void *s_lpsciHandle[];

/*! Pointer to lpsci IRQ handler for each instance. */
extern lpsci_isr_t s_lpsciIsr;
