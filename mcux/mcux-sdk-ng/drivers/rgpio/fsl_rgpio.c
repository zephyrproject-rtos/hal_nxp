...
uint32_t RGPIO_GetInstance(RGPIO_Type *base)
{
    ...
    {
        if (MSDK_REG_NONSECURE_ADDR(s_rgpioBases[instance]) == MSDK_REG_NONSECURE_ADDR(base))
        {
            break;
        }
    }
...
uint32_t FGPIO_GetInstance(FGPIO_Type *base)
{
    ...
    {
        if (MSDK_REG_NONSECURE_ADDR(s_fgpioBases[instance]) == MSDK_REG_NONSECURE_ADDR(base))
        {
            break;
        }
    }
