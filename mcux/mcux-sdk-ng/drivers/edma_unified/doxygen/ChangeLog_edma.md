...
- Improvements
  - Introduced the version-neutral name `driver.edma_unified` for the unified eDMA
    driver (which serves the eDMA3/eDMA4/eDMA5 IP versions), renamed the driver
    directory to `drivers/edma_unified`, and set `FSL_COMPONENT_ID` to
    `platform.drivers.edma_unified`. The previous `driver.edma4` Kconfig component
    and the `FSL_EDMA_DRIVER_EDMA4` macro are retained as deprecated aliases, so
    existing projects (including the shipped eDMA examples) keep building without
    changes; new projects should use `driver.edma_unified` / `FSL_EDMA_DRIVER_UNIFIED`.
    The C API, `edma_handle_t`, registers, and the IP-specific `edma4_*`/`edma5_*`
    TCD identifiers are unchanged.
