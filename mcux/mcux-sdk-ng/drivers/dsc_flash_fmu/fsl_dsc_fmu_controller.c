...
#include "fsl_dsc_fmu_controller.h"
...
/* This routine is needed to be able to address flash area that is mapped on addresses
 * with bitwidth more than 16 bits
 */
static inline void flash_write(register uint32_t ui32Addr, register uint32_t ui32Data)
{
    register uint32_t* ui32Ptr;
    asm
    {
        move.l ui32Addr, ui32Ptr
        move.l ui32Data, x:(ui32Ptr)
    }
}
...
static status_t flash_erase_sequence(FMU_Type *base, uint32_t start)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        /* Use DSC 16b alignment */
        flash_write(addr_8b/2, 0);
...
static status_t flash_pgm_sequence(FMU_Type *base, uint32_t start, uint32_t *src, uint8_t isPage)
{
    ...
    for (int i = 0; i < lengthInWord; i++)
    {
        /* Use DSC 16b alignment */
        flash_write(addr_8b/2, src[i]);
