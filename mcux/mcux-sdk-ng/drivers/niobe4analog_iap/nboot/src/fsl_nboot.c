...
#define NBOOT_API ((nboot_interface_t *)0x1302f9e4U)
...
{
    assert(NBOOT_API);
    return NBOOT_API->romapi_rng_generate_random(output, outputByteLen);
}
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_context_init(context);
}
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_context_deinit(context);
}
/*!
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_sb3_load_manifest(context, manifest, parms);
}
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_sb3_load_block(context, block);
}
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_img_authenticate_ecdsa(context, imageStartAddress, isSignatureVerified, parms);
}
...
{
    assert(NBOOT_API);
    return NBOOT_API->nboot_img_authenticate_cmac(context, imageStartAddress, isSignatureVerified, parms);
}
