...
void LPIT_Init(LPIT_Type *base, const lpit_config_t *config)
{
    ...
    base->MCR =
        (LPIT_MCR_DBG_EN(config->enableRunInDebug ? 1U : 0U) | LPIT_MCR_DOZE_EN(config->enableRunInDoze ? 1U : 0U) | LPIT_MCR_M_CEN_MASK);
}
...
status_t LPIT_SetupChannel(LPIT_Type *base, lpit_chnl_t channel, const lpit_chnl_params_t *chnlSetup)
{
    ...
    else
    {
        ...
        reg = (LPIT_TCTRL_MODE(chnlSetup->timerMode) | LPIT_TCTRL_TRG_SRC(chnlSetup->triggerSource) |
               LPIT_TCTRL_TRG_SEL(chnlSetup->triggerSelect) | LPIT_TCTRL_TROT(chnlSetup->enableReloadOnTrigger ? 1U : 0U) |
               LPIT_TCTRL_TSOI(chnlSetup->enableStopOnTimeout ? 1U : 0U) | LPIT_TCTRL_TSOT(chnlSetup->enableStartOnTrigger ? 1U : 0U) |
               LPIT_TCTRL_CHAIN(chnlSetup->chainChannel ? 1U : 0U));
