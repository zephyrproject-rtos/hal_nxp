...
#define FSL_HSADC_DRIVER_VERSION (MAKE_VERSION(2, 1, 0)) /*!< Version 2.1.0 */
...
/*!
 * @brief Gets the shifted sample result value.
 * 
 * @note This function returns the 13-bit right shifted value of the sample result value,
 * the sign bit is kept as the MSB
 * 
 * @param base              HSADC peripheral base address.
 * @param sampleIndex       Index of sample slot.
 * 
 * @return          Sample's shifted conversion value.
 */
static inline uint16_t HSADC_GetShiftedSampleResultValue(HSADC_Type *base, uint16_t sampleIndex)
{
    return (uint16_t)(((uint16_t)base->RSLT[sampleIndex]) >> (uint16_t)HSADC_RSLT_RSLT_SHIFT);
}
