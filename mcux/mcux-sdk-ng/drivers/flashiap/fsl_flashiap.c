...
{
    const int32_t status_max = (int32_t)kStatus_FLASHIAP_Max - MAKE_STATUS((int32_t)kStatusGroup_FLASHIAP, 0);
    ...
    else if (status >= (uint32_t)status_max)
    {
        return kStatus_Fail;
    }
    else
    {
        return MAKE_STATUS((int32_t)kStatusGroup_FLASHIAP, (int32_t)status);
    }
}
