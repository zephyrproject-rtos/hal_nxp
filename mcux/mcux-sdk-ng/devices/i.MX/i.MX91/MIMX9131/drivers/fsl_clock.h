...
static inline void CLOCK_SetRootClockMux(clock_root_t root, uint8_t src)
{
    ...
    CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW =
        (CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW & ~(CCM_CTRL_CLOCK_ROOT_MUX_MASK)) | CCM_CTRL_CLOCK_ROOT_MUX(src);
    __DSB();
    __ISB();
}
...
{
    return (CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW & CCM_CTRL_CLOCK_ROOT_MUX_MASK) >> CCM_CTRL_CLOCK_ROOT_MUX_SHIFT;
}
...
static inline void CLOCK_SetRootClockDiv(clock_root_t root, uint8_t div)
{
    assert(div);
    CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW =
        (CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW & ~CCM_CTRL_CLOCK_ROOT_DIV_MASK) |
        CCM_CTRL_CLOCK_ROOT_DIV((uint32_t)div - 1UL);
    __DSB();
    __ISB();
}
...
{
    return ((CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW & CCM_CTRL_CLOCK_ROOT_DIV_MASK) >> CCM_CTRL_CLOCK_ROOT_DIV_SHIFT) +
           1UL;
}
...
{
    if (0UL == (CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW & CCM_CTRL_CLOCK_ROOT_OFF_MASK))
    {
        CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.SET = CCM_CTRL_CLOCK_ROOT_OFF_MASK;
        __DSB();
        __ISB();
    }
}
...
{
    CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.CLR = CCM_CTRL_CLOCK_ROOT_OFF_MASK;
    __DSB();
    __ISB();
}
...
{
    assert(config);
    CCM_CTRL->CLOCK_ROOT[root].CLOCK_ROOT_CONTROL.RW = CCM_CTRL_CLOCK_ROOT_MUX(config->mux) |
                                                       CCM_CTRL_CLOCK_ROOT_DIV((uint32_t)config->div - 1UL) |
                                                       (config->clockOff ? CCM_CTRL_CLOCK_ROOT_OFF(config->clockOff) : 0UL);
    __DSB();
    __ISB();
}
...
{
    CCM_CTRL->LPCG[name].AUTHEN |= CCM_CTRL_LPCG_AUTHEN_CPULPM_MODE(1U);
    CCM_CTRL->LPCG[name].LPM_CUR = CCM_CTRL_LPCG_LPM_CUR_LPM_SETTING_CUR(value);
    __DSB();
    __ISB();
}
