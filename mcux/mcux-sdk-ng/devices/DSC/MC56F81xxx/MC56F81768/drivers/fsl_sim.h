...
{
    base->CTRL = (base->CTRL & ((uint16_t)(~SIM_CTRL_WAIT_DISABLE_MASK))) | SIM_CTRL_WAIT_DISABLE(eOperation);
}
...
{
    base->CTRL = (base->CTRL & ((uint16_t)(~SIM_CTRL_STOP_DISABLE_MASK))) | SIM_CTRL_STOP_DISABLE(eOperation);
}
...
{
    base->PWRMODE &= (uint16_t)(~SIM_PWRMODE_LPMODE_MASK);
}
...
{
    base->PWRMODE &= (uint16_t)(~SIM_PWRMODE_VLPMODE_MASK);
}
...
static inline void SIM_EnableResetPadCellInputFilter(SIM_Type *base, bool bEnable)
{
    ...
    {
        base->CTRL &= (uint16_t)(~SIM_CTRL_RST_FILT_MASK);
    }
}
...
{
    base->PWR = (base->PWR & ((uint16_t)(~SIM_PWR_SR12STDBY_MASK))) | SIM_PWR_SR12STDBY(eControlMode);
}
...
{
    base->PWR = (base->PWR & ((uint16_t)(~SIM_PWR_SR27_CONTROL_MODE_MASK))) | SIM_PWR_SR27_CONTROL_MODE(eControlMode);
}
...
{
    base->PWR = (base->PWR & ((uint16_t)(~SIM_PWR_LRSTDBY_MASK))) | SIM_PWR_LRSTDBY(eControlMode);
}
...
static inline void SIM_SetOnCEClockOperationMode(SIM_Type *base, sim_onceclk_operation_mode_t eOperationMode)
{
    ...
    {
        base->CTRL &= (uint16_t)(~SIM_CTRL_ONCEEBL_MASK);
    }
}
...
{
    base->CTRL = (base->CTRL & ((uint16_t)(~SIM_CTRL_DMAEBL_MASK))) | SIM_CTRL_DMAEBL(eOperationMode);
}
...
static inline void SIM_SetLPI2C1TriggerSelection(SIM_Type *base, sim_lpi2c_trigger_selection_t eTriggerSelection)
{
    ...
    {
        base->MISC0 &= (uint16_t)(~SIM_MISC0_LPI2C1_TRIG_SEL_MASK);
    }
    else
    {
...
static inline void SIM_SetLPI2C0TriggerSelection(SIM_Type *base, sim_lpi2c_trigger_selection_t eTriggerSelection)
{
    if (eTriggerSelection == kSIM_Lpi2cSlaveTrigger)
    {
        base->MISC0 &= (uint16_t)(~SIM_MISC0_LPI2C0_TRIG_SEL_MASK);
    }
    else
    {
...
static inline void SIM_SetDeviceOperateMode(SIM_Type *base, sim_device_operate_mode_t eOperateMode)
{
    ...
    {
        base->MISC0 &= (uint16_t)(~SIM_MISC0_FAST_MODE_MASK);
    }
    else
    {
...
static inline void SIM_EnableADCScanControlReorder(SIM_Type *base, bool bEnable)
{
    ...
    {
        base->MISC0 &= (uint16_t)(~SIM_MISC0_SCTRL_REORDER_MASK);
    }
}
...
static inline void SIM_SetMasterPIT(SIM_Type *base, sim_master_pit_selection_t eMasterPit)
{
    ...
    {
        base->MISC0 &= (uint16_t)(~SIM_MISC0_PIT_MSTR_MASK);
    }
    else
    {
...
#if defined(FSL_FEATURE_SIM_HAS_BOOT_MODE_OVERRIDE_REGISTER) && (FSL_FEATURE_SIM_HAS_BOOT_MODE_OVERRIDE_REGISTER == 1U)
/*!
...
/*!
 * @brief Reads device UID.
 *
 * @param base SIM peripheral base address.
 * @param pDevUID The pointer of external data buffer for device UID.
 */
static inline void SIM_ReadDeviceUID(SIM_Type *base, uint16_t *pDevUID)
{
    ...
    if (NULL != pDevUID)
    {
        volatile uint16_t *paddr = (volatile uint16_t *)(&(base->PWRMODE) + 0x9U);
