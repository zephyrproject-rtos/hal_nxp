...
typedef enum _clock_output_clk_src
{
    ...
    kCLOCK_OutputClkSrc_Firc    = 4U, /*!< Fast IRC clock, 8M/400K */
...
typedef enum _pit_count_clock_source
{
    ...
    kPIT_CountFircClk       = kPIT_CountClockSource2, /*!< PIT count clock sourced from fast IRC(8M/400K) clock */
...
typedef enum _ewm_lpo_clock_source
{
    ...
    /* This part is used for users. */
    kEWM_Lpo8MHz400KHzIRCClock = kEWM_LpoClockSource0, /*!< EWM clock sourced from 8MHz/400KHz IRC clock */
...
static inline void CLOCK_ConfigQtimerClockSrc(clock_ip_clk_src_t eClkSrc)
{
    if (kCLOCK_IPClkSrc_Bus2xClk == eClkSrc)
    {
        SIM->PCR |= SIM_PCR_TMR_MASK;
    }
    else
    {
        SIM->PCR &= ~((uint16_t)SIM_PCR_TMR_MASK);
    }
}
...
static inline void CLOCK_ConfigPWMClockSrc(clock_ip_clk_src_t eClkSrc)
{
    if (kCLOCK_IPClkSrc_Bus2xClk == eClkSrc)
    {
        SIM->PCR |= SIM_PCR_PWM_MASK;
    }
    else
    {
        SIM->PCR &= ~((uint16_t)SIM_PCR_PWM_MASK);
    }
}

/*!
 * @brief Configure I2C clock source.
 *
 * I2C clock could be bus or bus_2x clock. Default is bus clock.
 *
 * @param eClkSrc Clock source.
 */
static inline void CLOCK_ConfigI2cFilterClockSrc(clock_ip_clk_src_t eClkSrc)
{
    if (kCLOCK_IPClkSrc_Bus2xClk == eClkSrc)
    {
        SIM->PCR |= SIM_PCR_IIC_FILT_MASK;
    }
    else
    {
        SIM->PCR &= ~((uint16_t)SIM_PCR_IIC_FILT_MASK);
    }
}
