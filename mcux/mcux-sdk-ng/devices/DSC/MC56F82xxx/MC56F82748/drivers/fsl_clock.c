...
#define OCCS_INTERNAL_FIRC_S_FREQ 400000UL  /* FIRC standby mode, 400K */
 ...
 * return clock mode
 */
static inline clock_mode_t CLOCK_GetClockMode(void)
{
    clock_mode_t mode = kCLOCK_Mode_Normal;
...
uint32_t CLOCK_GetFreq(clock_name_t eClkName)
{
    ...
    switch (eClkName)
    {
        ...
        case kCLOCK_BusClk:
            u32Freq = CLOCK_GetMstr2xFreq() / 2U;
            if (kCLOCK_Mode_Fast == CLOCK_GetClockMode())
            {
                /* fast mode */
                u32Freq /= 2U;
            }
...
uint32_t CLOCK_GetIpClkSrcFreq(clock_ip_name_t eIpClkName)
{
    ...
    switch (eIpClkName)
    {
        ...
        case kCLOCK_TA3:
            ...
            if ((SIM->PCR & SIM_PCR_TMR_MASK) == 0U)
            {
                /* Not 2X clock */
                u32Freq /= 2U;
            }
        ...
        case kCLOCK_PWMACH3:
            u32Freq = CLOCK_GetFreq(kCLOCK_Bus2xClk);
            if ((SIM->PCR & SIM_PCR_PWM_MASK) == 0U)
            {
                /* Not 2X clock */
                u32Freq /= 2U;
            }
...
void CLOCK_SetClkConfig(clock_config_t *psConfig)
{
    ...
    if (psConfig->bPllEnable)
    {
        ...
        {
            OCCS->DIVBY &= ~((uint16_t)OCCS_DIVBY_PWM_DIV2_MASK);
        }
        else
        {
...
{
    if (eClkMode != CLOCK_GetClockMode())
    {
#if (defined(FSL_FEATURE_SIM_MISC0_HAS_MODE_STAT_BIT_FIELD) && FSL_FEATURE_SIM_MISC0_HAS_MODE_STAT_BIT_FIELD)
 ...
 * 1. FIRC accuracy, now it is -5% to +4% for full temperature range.
...
uint32_t CLOCK_EvaluateExtClkFreq(void)
{
    ...
    assert(0U == (OCCS->OSCTL1 & (OCCS_OSCTL1_ROPD_MASK | OCCS_OSCTL1_ROSB_MASK)));
...
void OCCS_DriverIRQHandler(void)
{
    ...
    if (OCCS_CTRL_PRECS(kCLOCK_MstrOscClkSrcExt) == (OCCS->CTRL & OCCS_CTRL_PRECS_MASK))
    {
        ...
        if (kCLOCK_Mode_Fast == CLOCK_GetClockMode())
        {
            ...
            OCCS->DIVBY = (OCCS->DIVBY & (~((uint16_t)OCCS_DIVBY_COD_MASK))) | OCCS_DIVBY_PWM_DIV2_MASK;
        }
        else
        {
            /* Normal mode, Output of PLL is 400MHz, Mstr2x clk = 100M, SYS_CLK = 50MHz */
            OCCS->DIVBY =
                (OCCS->DIVBY & (~((uint16_t)OCCS_DIVBY_COD_MASK))) | OCCS_DIVBY_COD(1U) | OCCS_DIVBY_PWM_DIV2_MASK;
        }
