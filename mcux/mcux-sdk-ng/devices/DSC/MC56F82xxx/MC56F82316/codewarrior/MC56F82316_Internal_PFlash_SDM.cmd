...
FLASH_PARTITION {
  ...
  FlashImage   : ORIGIN_P = 0x0, ORIGIN_X = 0x4000, LENGTH = 0x2000 > FlashImage.bin
  # FlashPart1 : ORIGIN_P = 0x0, ORIGIN_X = 0x4000, LENGTH = 0x2000
}
...
MEMORY {
  ...
  .p_Code       (RWX): ORIGIN = 0x208,   LENGTH = 0x2000 - 0x208 - 0x200  # XIP(excute in place) code resides in flash
  .x_FlashData   (RX): ORIGIN = AFTER(.p_Code) + 0x4000, LENGTH = 0  # Flash data stored in flash but accessed from x space

  .p_RamCode     (RX): ORIGIN = 0xF000, LENGTH = 0x800           # RAM mapped into p space, code resides in flash and runs in RAM

  .p_RamData     (RX): ORIGIN = AFTER(.p_RamCode) - 0xF000, LENGTH = 0x800 + 0xF000 - AFTER(.p_RamCode)  # .data section
  .x_Data        (RW): ORIGIN = AFTER(.p_RamCode) - 0xF000, LENGTH = 0x800 + 0xF000 - AFTER(.p_RamCode)  # bss, heap, stack ...

  .p_NVM        (RWX): ORIGIN = 0x2000 - 0x200, LENGTH = 0x200    # Use flash last sector as NVM, stored and accessed in p space
}
