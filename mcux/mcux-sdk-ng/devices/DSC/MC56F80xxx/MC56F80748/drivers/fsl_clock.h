...
typedef enum _clock_ip_name
{
    ...
    /* SIM_PCE2 */
    kCLOCK_PIT2   = 32U + 1U,  /*!< PIT 2 clock */
...
{
    kCLOCK_Mode_Normal = 0, /*!< Normal mode, bus:flash clock rate = 2:1 */
    kCLOCK_Mode_Fast   = 1  /*!< Fast mode, bus:flash clock rate = 4:1 */
} clock_mode_t;
...
#define CMP_CLOCKS                            \
    {                                         \
        kCLOCK_CMPA, kCLOCK_CMPB, kCLOCK_CMPC \
    }
 ...
 * @param eClkSrc Clock source.
 */
static inline void CLOCK_ConfigI2cClockSrc(clock_ip_clk_src_t eClkSrc)
{
    if (0U == (SIM->MISC0 & SIM_MISC0_MODE_STAT_MASK))
    {
        /* LPI2C could be configured only when normal mode */
        if (kCLOCK_IPClkSrc_Bus2xClk == eClkSrc)
        {
            SIM->PCR |= SIM_PCR_LPI2C0_CR_MASK;
        }
        else
        {
            SIM->PCR &= ~SIM_PCR_LPI2C0_CR_MASK;
