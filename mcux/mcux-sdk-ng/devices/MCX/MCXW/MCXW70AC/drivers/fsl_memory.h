...
#define FSL_MEM_TCM_BEGIN 0x20000000U
#define FSL_MEM_TCM_END 0x2003FFFFU
...
static inline uint32_t MEMORY_ConvertMemoryMapAddress(uint32_t addr, mem_direction_t direction)
{
    ...
    switch (direction)
    {
        ...
        {
            if (((addr >= FSL_MEM_TCM_BEGIN) && (addr <= FSL_MEM_TCM_END)) || ((addr > 0x40000000) && (addr < 0x4FFFFFFF)))
            {
                dest = addr | 0x10000000;
            }
            else
            {
                dest = addr;
