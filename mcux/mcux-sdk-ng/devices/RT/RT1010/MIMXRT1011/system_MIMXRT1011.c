...
void SystemCoreClockUpdate (void) {
    ...
    else
    {
        switch (CCM->CBCMR & CCM_CBCMR_PRE_PERIPH_CLK_SEL_MASK)
        {
            ...
            {
                uint64_t tmpFreq = (uint64_t)PLL3MainClock /
                    (uint64_t)((CCM_ANALOG->PFD_480 & CCM_ANALOG_PFD_480_PFD3_FRAC_MASK) >> CCM_ANALOG_PFD_480_PFD3_FRAC_SHIFT) * 18UL;
            ...
            case CCM_CBCMR_PRE_PERIPH_CLK_SEL(2U):
            {
                uint64_t tmpFreq = (uint64_t)PLL2MainClock /
                    (uint64_t)((CCM_ANALOG->PFD_528 & CCM_ANALOG_PFD_528_PFD3_FRAC_MASK) >> CCM_ANALOG_PFD_528_PFD3_FRAC_SHIFT) * 18UL;
                freq = (tmpFreq > (uint64_t)UINT32_MAX) ? UINT32_MAX : (uint32_t)tmpFreq;
                break;
            }
