...
#define ADC_SAR_IP_SW_MAJOR_VERSION_C               0
#define ADC_SAR_IP_SW_MINOR_VERSION_C               8
...
#define ADC_SAR_IP_VALID_CTYPE(cType)       (((cType) == ADC_SAR_IP_DATA_CONV_SINGLE_ENDED) ||     \
                                             ((cType) == ADC_SAR_IP_DATA_CONV_DIFFERENTIAL_P_M) || \
                                             ((cType) == ADC_SAR_IP_DATA_CONV_DUAL_SINGLE_ENDED) )
/*==================================================================================================
*                                      LOCAL CONSTANTS
==================================================================================================*/
...
static inline Adc_Sar_Ip_StatusType Adc_Sar_ComputeCapacitorCalibrationValue(uint32 GccCalibRaw,
                                                                             uint32 * GccCalibAdjusted);
static inline Adc_Sar_Ip_StatusType Adc_Sar_PerformGCRUpdate(volatile const uint32 * GCCAddr,
                                                             volatile uint32 * GCRAddr);
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckCapCalibrationStatus(volatile const uint32 *MSRAddr);
static inline Adc_Sar_Ip_StatusType Adc_Sar_PerformCapacitorCalibration(uint32 u32Instance);
                                                   ...
                                                   const Adc_Sar_Ip_SelfTestThresholdType * SelfTestThresholdConfig)
{
    ...
    ADC_Type * AdcBasePtr = NULL_PTR;
    volatile uint32 * STAWVBGAddr = NULL_PTR;
    volatile uint32 * STAWVDDIAddr = NULL_PTR;
    volatile uint32 * STAWVDDFAddr = NULL_PTR;
    volatile uint32 * STAWVRHLAddr = NULL_PTR;
    volatile uint32 * STAWOFSAddr = NULL_PTR;
    volatile uint32 * STAWCAPMAddr = NULL_PTR;
    volatile uint32 * STAWCAPIAddr = NULL_PTR;
    volatile uint32 * STAWCAPLAddr = NULL_PTR;

    AdcBasePtr = Adc_Sar_Ip_apxAdcBase[Instance];

    STAWVBGAddr  = (volatile uint32 *)&(AdcBasePtr->STAWVBG);
    STAWVDDIAddr = (volatile uint32 *)&(AdcBasePtr->STAWVDDI);
    STAWVDDFAddr = (volatile uint32 *)&(AdcBasePtr->STAWVDDF);
    STAWVRHLAddr = (volatile uint32 *)&(AdcBasePtr->STAWVRHL);
    STAWOFSAddr  = (volatile uint32 *)&(AdcBasePtr->STAWOFS);
    STAWCAPMAddr = (volatile uint32 *)&(AdcBasePtr->STAWCAPM);
    STAWCAPIAddr = (volatile uint32 *)&(AdcBasePtr->STAWCAPI);
    STAWCAPLAddr = (volatile uint32 *)&(AdcBasePtr->STAWCAPL);

    /* Configure the self-test watchdog threshold for VBG supply algorithm step */
    *STAWVBGAddr = 0UL;
    *STAWVBGAddr |= SARADC_STAWVBG_THRHVBG(SelfTestThresholdConfig->AdcSTAWVBGHighVal) | SARADC_STAWVBG_THRLVBG(SelfTestThresholdConfig->AdcSTAWVBGLowVal);
    /* Configure the self-test watchdog threshold for VDDI supply algorithm step */
    *STAWVDDIAddr = 0UL;
    *STAWVDDIAddr |= SARADC_STAWVDDI_THRHVDDI(SelfTestThresholdConfig->AdcSTAWVDDIHighVal) | SARADC_STAWVDDI_THRLVDDI(SelfTestThresholdConfig->AdcSTAWVDDILowVal);
    /* Configure the self-test watchdog threshold for VDDF supply algorithm step */
    *STAWVDDFAddr = 0UL;
    *STAWVDDFAddr |= SARADC_STAWVDDF_THRHVDDF(SelfTestThresholdConfig->AdcSTAWVDDFHighVal) | SARADC_STAWVDDF_THRLVDDF(SelfTestThresholdConfig->AdcSTAWVDDFLowVal);
    /* Configure the self-test watchdog threshold for VREFH supply algorithm step */
    *STAWVRHLAddr = 0UL;
    *STAWVRHLAddr |= SARADC_STAWVRHL_VREFL(SelfTestThresholdConfig->AdcSTAWVRHLLowVal) | SARADC_STAWVRHL_VRCORXN(SelfTestThresholdConfig->AdcSTAWVRHLCorrVal) | SARADC_STAWVRHL_THRLVRHL(SelfTestThresholdConfig->AdcSTAWVRHLCompVal);
    /* Configure the self-test watchdog threshold for offset algorithm */
    *STAWOFSAddr = 0UL;
    *STAWOFSAddr |= SARADC_STAWOFS_THRHOFS(SelfTestThresholdConfig->AdcSTAWOFSHighVal) | SARADC_STAWOFS_THRLOFS(SelfTestThresholdConfig->AdcSTAWOFSLowVal);
    /* Configure the self-test watchdog threshold for MSB capacitor algorithm step*/
    *STAWCAPMAddr = 0UL;
    *STAWCAPMAddr |= SARADC_STAWCAPM_THRHCAPM(SelfTestThresholdConfig->AdcSTAWCAPMHighVal) | SARADC_STAWCAPM_THRLCAPM(SelfTestThresholdConfig->AdcSTAWCAPMLowVal);
    /* Configure the self-test watchdog threshold for ISB capacitor algorithm step*/
    *STAWCAPIAddr = 0UL;
    *STAWCAPIAddr |= SARADC_STAWCAPI_THRHCAPI(SelfTestThresholdConfig->AdcSTAWCAPIHighVal) | SARADC_STAWCAPI_THRLCAPI(SelfTestThresholdConfig->AdcSTAWCAPILowVal);
    /* Configure the self-test watchdog threshold for LSB capacitor algorithm step*/
    *STAWCAPLAddr = 0UL;
    *STAWCAPLAddr |= SARADC_STAWCAPL_THRHCAPL(SelfTestThresholdConfig->AdcSTAWCAPLHighVal) | SARADC_STAWCAPL_THRLCAPL(SelfTestThresholdConfig->AdcSTAWCAPLLowVal);
}
/*FUNCTION*********************************************************************
 *
...
static inline void Adc_Sar_EnableSelftestThreshold(const uint32 Instance)
{
    ...
    /* enable self-test watchdog thresholds for algorithms S, O and P when STC2[ALGO] is 3 */
    *STCR1Addr |= SARADC_STC1_AWDEVBG(0x1UL) | SARADC_STC1_AWDEVDD(0x1UL) | SARADC_STC1_AWDEVRHL(0x1UL) | SARADC_STC1_AWDEOFS(0x1UL) | SARADC_STC1_AWDECAP(0x1UL);
...
static inline void Adc_Sar_DisableSelftestThreshold(const uint32 Instance)
{
    ...
    AdcBasePtr = Adc_Sar_Ip_apxAdcBase[Instance];
    STCR1Addr = (volatile uint32 *)&(AdcBasePtr->STCR1);
    /* disable self-test watchdog thresholds for algorithms S, O and P when STC2[ALGO] is 3 */
    *STCR1Addr &= ~(SARADC_STC1_AWDEVBG_MASK | SARADC_STC1_AWDEVDD_MASK | SARADC_STC1_AWDEVRHL_MASK | SARADC_STC1_AWDEOFS_MASK | SARADC_STC1_AWDECAP_MASK);
#endif /* ADC_SAR_IP_HAS_SELFTEST_STCR1 */
}
#endif /* (STD_ON == ADC_SAR_IP_SELFTEST_ENABLED) */
...
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckSelfTestProgress(const uint32 Instance)
{
    ...
    volatile uint32 * STS1PAddr = NULL_PTR;
    volatile uint32 * STS1MAddr = NULL_PTR;
    const uint32 AlgoMask[3] = {SARADC_STS1_P_WDGEOA_S_MASK, SARADC_STS1_P_WDGEOA_O_MASK, SARADC_STS1_P_WDGEOA_C_MASK};

    {
        AdcBasePtr = Adc_Sar_Ip_apxAdcBase[Instance];
        MSRAddr = (volatile const uint32 *)&(AdcBasePtr->MSR);
        STS1PAddr = (volatile uint32 *)&(AdcBasePtr->STS1_P);
        STS1MAddr = (volatile uint32 *)&(AdcBasePtr->STS1_M);
    }

     /*  Each for loop checks the conversion of all steps in self-test S algorithm
    *   ForLoop(0): wait for S algorithm to finish
    *   ForLoop(1): wait for O algorithm to finish
    *   ForLoop(2): wait for C algorithm to finish
    */
    for (Index = 0U; Index < 3U; Index++)
    {
        ...
        {
            Status = Adc_Sar_CheckFlagWithTimeout(STS1PAddr, AlgoMask[Index], AlgoMask[Index]);
            ...
            Status = Adc_Sar_CheckFlagWithTimeout(STS1MAddr, AlgoMask[Index], AlgoMask[Index]);
            if (Status == ADC_SAR_IP_STATUS_TIMEOUT)
            {
                /* Break the loop if timeout occured in order to increase performance */
                break;
    ...
    if (Status != ADC_SAR_IP_STATUS_TIMEOUT)
    {
        Reg |= SARADC_STS1_P_ERRVBG_MASK | SARADC_STS1_P_ERRVDD_MASK | SARADC_STS1_P_ERRVRTST_MASK |
               SARADC_STS1_P_ERROFS_MASK | SARADC_STS1_P_ERRCAP_MASK;


        uint32 STS1PAddrTemp = *STS1PAddr;
        uint32 STS1MAddrTemp = *STS1MAddr;
        if (((STS1PAddrTemp & Reg) != 0UL) || ((STS1MAddrTemp & Reg) != 0UL))
        {
        ...
        /* clear error flags */
        Reg = 0;
        Reg |= SARADC_STS1_P_ERRVBG(0x01U) | SARADC_STS1_P_ERRVDD(0x01U) | SARADC_STS1_P_ERRVRTST(0x01U) |
               SARADC_STS1_P_ERROFS(0x01U) | SARADC_STS1_P_ERRCAP(0x01U) | SARADC_STS1_P_OVERWR(0x01U) | SARADC_STS1_P_WDTERR(0x01U);
        *STS1PAddr = Reg;
        *STS1MAddr = Reg;
    }
    /* clear self test status flags in case of a timeout occurence */
    Reg = 0;
    Reg |= SARADC_STS1_P_WDGEOA_S(0x01U) | SARADC_STS1_P_WDGEOA_O(0x01U) | SARADC_STS1_P_WDGEOA_C(0x01U) |
           SARADC_STS1_P_STEOCSUP(0x01U) | SARADC_STS1_P_STEOCOFS(0x01U) | SARADC_STS1_P_STEOCCAP(0x01U);
    *STS1PAddr = Reg;
    *STS1MAddr = Reg;
    return Status;
}
...
#endif /* ADC_SAR_IP_HAS_ADCLKSEL */
#if ADC_SAR_IP_HAS_ADCLKDIV
        Mcr |= ADC_MCR_ADCLKDIV(1u);
#endif /* ADC_SAR_IP_HAS_ADCLKDIV */
...
#endif /* defined(ADC_CAL2_ENX_MASK) */
{
    ...
    Calbistreg &= ~(ADC_SAR_IP_CALBISTREG_CST_LONG_MASK | ADC_SAR_IP_CALBISTREG_NR_SMPL_MASK | ADC_SAR_IP_CALBISTREG_AVG_EN_MASK | ADC_SAR_IP_CALBISTREG_TEST_EN_MASK | ADC_SAR_IP_CALBISTREG_TEST_CAP_EN_MASK);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_ComputeCapacitorCalibrationValue
 * Description   : This function perfrom computation and adjustment of capacitor calibration values.
 *
 *END**************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_ComputeCapacitorCalibrationValue(uint32 GccCalibRaw, uint32 * GccCalibAdjusted)
{
    Adc_Sar_Ip_StatusType Status = ADC_SAR_IP_STATUS_SUCCESS;
    sint32 GccCalibExtendSign = 0;
    uint32 divisor = 0;
    uint32 integerPart = 0;
    uint32 fractionalPart = 0;
    uint32 result = 0;

    sint32 TempGccCalibRawSigned = (sint32)GccCalibRaw; /* Use intermediate variable in order to avoid Misra 10.8 */

    uint32 TempGccCalibRawUnsigned = (uint32)TempGccCalibRawSigned << 15; /* Cast to unsigned in order to avoid Misra 10.1 */
    TempGccCalibRawSigned = (sint32)TempGccCalibRawUnsigned;

    TempGccCalibRawUnsigned = (uint32)TempGccCalibRawSigned >> 15;

    /* Sign extend from 17 to 32 bits */
    GccCalibExtendSign = (sint32)TempGccCalibRawUnsigned;

    /* Use intermediate variable in order to avoid Misra 10.8 */
    sint32 DivisorSigned = 65536 - GccCalibExtendSign;
    divisor = (uint32)DivisorSigned;
    /* Check for valid divisor to prevent division by zero */
    if (divisor <= 0U)
    {
        *GccCalibAdjusted =  0xFFFFFFFFU; /* Default to max value in case of invalid divisor */
        Status = ADC_SAR_IP_STATUS_ERROR;
    }
    else
    {
        result = (uint32)((uint64)(65536ULL * 65536ULL) / (uint32)divisor);

        integerPart = (uint32)(result >> 16U);
        fractionalPart = (uint32)(result & 0xFFFFU);

        *GccCalibAdjusted = (integerPart << 16U) | fractionalPart;
    }
    return Status;
}
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_PerformGCRUpdate
 * Description   : This function perfrom update to the GCR_P/M register with the new calibration value.
 *
 *END**************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_PerformGCRUpdate(volatile const uint32 * GCCAddr, volatile uint32 * GCRAddr)
{
    Adc_Sar_Ip_StatusType Status = ADC_SAR_IP_STATUS_SUCCESS;
    uint32 GccCalibRaw =  0u;
    uint32 GccCalibAdjusted =  0u;

     /* wait for the GCC_P[VALID] to assert*/
    Status = Adc_Sar_CheckFlagWithTimeout(GCCAddr, SARADC_GCC_P_VALID_MASK, SARADC_GCC_P_VALID_MASK);
    if (ADC_SAR_IP_STATUS_SUCCESS == Status)
    {
        /* compute and save GCC P calibration adjusted value */
        GccCalibRaw = (uint32)((*GCCAddr) & SARADC_GCC_P_GAIN_CAL_MASK);
        Status = Adc_Sar_ComputeCapacitorCalibrationValue(GccCalibRaw, &GccCalibAdjusted);

        if (ADC_SAR_IP_STATUS_SUCCESS == Status)
        {
            *GCRAddr =  SARADC_GCR_P_GCALR(GccCalibAdjusted);
        }
    }
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_CheckCapCalibrationStatus
 * Description   : This function check the final status flags for capacitor calibration.
 *
 *END**************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_CheckCapCalibrationStatus(volatile const uint32 *MSRAddr)

{
    ...
    /* wait and check the MS[CAP_DONE] flag */
    Status = Adc_Sar_CheckFlagWithTimeout(MSRAddr, ADC_MSR_CAP_DONE_MASK, ADC_MSR_CAP_DONE_MASK);

    /* Check the MS[CALFAIL] flag for calibration failure */
    uint32 MSRAddrTemp = *MSRAddr;
    if ((ADC_SAR_IP_STATUS_SUCCESS == Status) && ((MSRAddrTemp & ADC_MSR_CALFAIL_MASK) != 0U))
    {
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Adc_Sar_PerformCapacitorCalibration
 * Description   : This function perfrom the capacitor calibration algorithm.
 *
 *END**************************************************************************/
static inline Adc_Sar_Ip_StatusType Adc_Sar_PerformCapacitorCalibration(uint32 u32Instance)
{
    ...
    volatile const uint32 * MSRAddr = NULL_PTR;
    volatile const uint32 * GCCPAddr = NULL_PTR;
    volatile uint32 * GCRPAddr = NULL_PTR;
    volatile const uint32 * GCCMAddr = NULL_PTR;
    volatile uint32 * GCRMAddr  = NULL_PTR;
    volatile uint32 * CALBISTREGAddr = NULL_PTR;

    AdcBasePtr = Adc_Sar_Ip_apxAdcBase[u32Instance];
    MSRAddr = (volatile const uint32 *)&(AdcBasePtr->MSR);
    GCCPAddr = (volatile const uint32 *)&(AdcBasePtr->GCC_P);
    GCRPAddr = (volatile uint32 *)&(AdcBasePtr->GCR_P);
    GCCMAddr = (volatile const uint32 *)&(AdcBasePtr->GCC_M);
    GCRMAddr = (volatile uint32 *)&(AdcBasePtr->GCR_M);
    CALBISTREGAddr = (volatile uint32 *)&(AdcBasePtr->CALBISTREG);

    SchM_Enter_Adc_ADC_EXCLUSIVE_AREA_36();
    /* Start capacitor calibration */
    *CALBISTREGAddr |= ADC_SAR_IP_CALBISTREG_TEST_CAP_EN(1U);
    SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_36();

    CalStatus = Adc_Sar_PerformGCRUpdate(GCCPAddr, GCRPAddr);

    if (ADC_SAR_IP_STATUS_SUCCESS == CalStatus)
    {
        CalStatus =  Adc_Sar_PerformGCRUpdate(GCCMAddr, GCRMAddr);

        if (ADC_SAR_IP_STATUS_SUCCESS == CalStatus)
        {
             /* set the GCR_X[VALID] bit */
            *GCRPAddr |= SARADC_GCR_P_VALID(0x01U);
            *GCRMAddr |= SARADC_GCR_P_VALID(0x01U);

            CalStatus = Adc_Sar_CheckCapCalibrationStatus(MSRAddr);
        }
        else
        {
            ; /* Empty else branch to avoid MISRA */
...
Adc_Sar_Ip_StatusType Adc_Sar_Ip_SelfTest(const uint32 u32Instance)
{
    ...
    if (FALSE == Adc_Sar_Ip_axAdcSarState[u32Instance].CalibrationOnGoing)
    {
        ...
        /* 1. Enable the self-testing channel by setting STCR2[EN].*/
...
#if ADC_SAR_IP_HAS_ADCLKDIV
            AdcClkSel |= ((*MCRAddr) & ADC_MCR_ADCLKDIV_MASK);
            *MCRAddr &= ~(ADC_MCR_ADCLKDIV_MASK);
            *MCRAddr |= ADC_MCR_ADCLKDIV(1U);
#endif /* ADC_SAR_IP_HAS_ADCLKDIV*/
            SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_11();
                ...
                /* 2. Program NCMR0 to select channels to be converted for normal conversion.*/
                ...
                *MCRAddr |= ADC_MCR_MODE(0x1UL);
                /* 4. Configure MC Register for selftest */
                *MCRAddr &=~(ADC_MCR_AVGEN_MASK | ADC_MCR_AVGS_MASK | ADC_MCR_CTYPE_MASK | ADC_MCR_EDGE_MASK | ADC_MCR_TRGEN_MASK);
                SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_11();
                SchM_Enter_Adc_ADC_EXCLUSIVE_AREA_35();
                /* 5.  Select the self-testing algorithm in STCR2[ALG] */
                *STCR2Addr |= ADC_STCR2_ALG(0x3UL);
                SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_35();

                Adc_Sar_EnableSelftestThreshold(u32Instance);
                /* 6. Power up the ADC instance */
                Status = Adc_Sar_Ip_Powerup(u32Instance);
                if (ADC_SAR_IP_STATUS_SUCCESS == Status)
                {
        ...
        CAL2Addr = (volatile uint32 *)&(AdcBasePtr->CAL2);
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
                {
                    /* Wait for MS[OFS_DONE] to assert */
                    CalStatus = Adc_Sar_CheckFlagWithTimeout(MSRAddr, ADC_MSR_CALIBRTD_MASK, ADC_MSR_CALIBRTD_MASK);
                    if (ADC_SAR_IP_STATUS_SUCCESS == CalStatus)
                    {
                        /* wait for CALBIST[OFS_REQ] to clear */
                        CalStatus = Adc_Sar_CheckFlagWithTimeout(CALBISTREGAddr, ADC_SAR_IP_CALBISTREG_TEST_EN_MASK, 0U);
                        uint32 TempMSRAddr = *MSRAddr;
                        if ((ADC_SAR_IP_STATUS_SUCCESS == CalStatus) && ((TempMSRAddr & ADC_MSR_CALFAIL_MASK) != 0U))
                        {
                            /* If the calibration failed, return error */
                            CalStatus = ADC_SAR_IP_STATUS_ERROR;
                        }
                    ...
                    /* wait for capacitor calibration to finish*/
                    if (ADC_SAR_IP_STATUS_SUCCESS == CalStatus)
                    {
                        CalStatus = Adc_Sar_PerformCapacitorCalibration(u32Instance);
                    }
                    else
                    {
                        ; /* Empty else branch to avoid MISRA */
...
#endif /* (!ADC_SAR_IP_CALIBRATION_USES_MCR || ADC_SAR_IP_APPLICATION_EXTENSION_AVAILABLE) */
            {
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)*/
                {
                    ...
                    {
                        uint32 MsrAddrTemp = *MSRAddr;
                        if ((MsrAddrTemp & ADC_MSR_CALFAIL_MASK) != 0U)
                        {
                            /* If the calibration failed, return error */
                            CalStatus = ADC_SAR_IP_STATUS_ERROR;
                        }
...
/*FUNCTION**********************************************************************
*
* Function Name : Adc_Sar_Ip_SetDataConvMode
* Description   : This function configures the used data conversion mode.
*
* @implements      Adc_Sar_Ip_SetDataConvMode_Activity
* END**************************************************************************/
Adc_Sar_Ip_StatusType Adc_Sar_Ip_SetDataConvMode(const uint32 u32Instance,
                                  const Adc_Sar_Ip_DataConvType eCType)
{
    Adc_Sar_Ip_StatusType Status    = ADC_SAR_IP_STATUS_ERROR;
    ADC_Type *AdcBasePtr            = NULL_PTR;
    volatile const uint32 *MSRAddr  = NULL_PTR;
    volatile uint32 *MCRAddr        = NULL_PTR;
...
#if (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(u32Instance < ADC_SAR_IP_INSTANCE_COUNT);
    DevAssert(ADC_SAR_IP_VALID_CTYPE(eCType));
#endif /* (ADC_SAR_IP_DEV_ERROR_DETECT == STD_ON) */
...
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    {
        ...
        if(((*MSRAddr) & ADC_MSR_ADCSTATUS_MASK) == ADC_MSR_ADCSTATUS(ADC_SAR_IP_MSR_ADCSTATUS_IDLE))
        {
            /* Edit CTYPE */
            SchM_Enter_Adc_ADC_EXCLUSIVE_AREA_19();
            *MCRAddr &= ~(SARADC_MC_CTYPE_MASK); /* clear previous CTYPE */
            *MCRAddr |= SARADC_MC_CTYPE(eCType); /* set new conversion type */
            SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_19();

            Status = ADC_SAR_IP_STATUS_SUCCESS;
        }
    }
