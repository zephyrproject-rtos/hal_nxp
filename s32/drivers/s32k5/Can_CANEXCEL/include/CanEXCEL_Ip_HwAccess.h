...
#define CANEXCEL_IP_HWACCESS_AR_RELEASE_MINOR_VERSION_H       9
#define CANEXCEL_IP_HWACCESS_AR_RELEASE_REVISION_VERSION_H    0
#define CANEXCEL_IP_HWACCESS_SW_MAJOR_VERSION_H               0
#define CANEXCEL_IP_HWACCESS_SW_MINOR_VERSION_H               8
...
#define CANXL_TX_HEADER_SEC_MASK                               0x80000u
#define CANXL_TX_HEADER_SEC_SHIFT                              19

#define CANXL_RX_HEADER_XLF_MASK                              (0x40000000U)
#define CANXL_RX_HEADER_FDF_MASK                              (0x80000000U)
#define CANXL_RX_HEADER_IDE_MASK                              (0x40000000U)
#define CANXL_RX_HEADER_BRS_MASK                              (0x20000000U)
#define CANXL_RX_HEADER_SEC_MASK                              (0x00080000U)


#define CANXL_RX_HEADER_SDT_MASK                              (0x0FF00000U)
#define CANXL_RX_HEADER_SDT_SHIFT                             (20U)

#define CANXL_RX_HEADER_VCID_MASK                              (0x000000FFU)
#define CANXL_RX_HEADER_VCID_SHIFT                             (0U)
...
typedef enum
{
    ...
    CANXL_INT_MRU         = CANXL_SIC_SYSIE_MRUIE_MASK         /*!< Message Receive Unit Interrupt */
} canxl_int_type_t;
 ...
 */
void CanXL_SetTxMsgBuffData(const Canexcel_Ip_TxDataInfoType * info, const uint8 * data, uint32 * MB);
/*!
...
uint8 CAN_ComputePayloadSizeHw(uint8 dlcValue);
 /*!
...
{
    base->CANXL_IP_TBS_SELECTOR |= CAN_TBS_CAN_TS_SEL_TS_SEL(type);
}
 ...
 */
static inline void CanXL_ClearMsgDescIntStatusFlag(CANXL_GRP_CONTROL_Type * base, uint8 msgBuffIdx)
{
 ...
 */
static inline uint8 CanXL_GetMsgDescIntStatusFlag(const CANXL_GRP_CONTROL_Type * base, uint8 msgBuffIdx)
{
 ...
 * @return  The individual Message Descriptor interrupt flag (0 and 1 are the flag value)
 */
static inline uint8 CanXL_GetMsgBuffIntStatusFlag(const CANXL_GRP_CONTROL_Type * base, uint8 msgBuffIdx)
{
    /* TODO: This need to be protected multithread access*/
    uint8 flag = 0;
    uint32 mask;
...
/*!
 * @brief Return RX FIFO State and data transfer pointers status
 *
 * @param   base  The CanEXCEL base address
 * @param   SysPtrIdx  The drescriptor System index Value
 * @param   HwPtrIdx  The drescriptor HW index Value
 *
 * @return  The RX FIFO State
 * enumerated by Canexcel_Ip_DescState type
 */
static inline Canexcel_Ip_RxFifoState CanXL_GetRxFifoStatus(const CANXL_RXFIFO_CONTROL_Type * base, uint8 * SysPtrIdx, uint8 * HwPtrIdx)
{
    Canexcel_Ip_RxFifoState retval;
    uint32 RXFCSTA = base->RXFCSTA;

    switch (RXFCSTA & CANXL_RXFIFO_CONTROL_RXFCSTA_STATE_MASK)
    {
        case 0u:
            retval = CANEXCEL_FIFO_STATE_INACTIVE;
            break;
        case 1u:
            retval = CANEXCEL_FIFO_STATE_EMPTY;
            break;
        case 2u:
            retval = CANEXCEL_FIFO_STATE_NOTFULL;
            break;
        case 3u:
            retval = CANEXCEL_FIFO_STATE_FULL;
            break;
        default:
            retval = CANEXCEL_FIFO_STATE_OVERRUN;
            break;
    }

    if (NULL_PTR != SysPtrIdx)
    {
        *SysPtrIdx = (uint8)((RXFCSTA & CANXL_RXFIFO_CONTROL_RXFCSTA_SYSLOCK_MASK) >> CANXL_RXFIFO_CONTROL_RXFCSTA_SYSLOCK_SHIFT);
    }

    if (NULL_PTR != HwPtrIdx)
    {
        *HwPtrIdx = (uint8)((RXFCSTA & CANXL_RXFIFO_CONTROL_RXFCSTA_HWPOINTER_MASK) >> CANXL_RXFIFO_CONTROL_RXFCSTA_HWPOINTER_SHIFT);
    }
 ...
 * enumerated by Canexcel_Ip_DescState type
 */
static inline Canexcel_Ip_DescState CanXL_GetDesciptorStatus(const CANXL_DSC_CONTROL_Type * base, uint8 descNo, uint8 * SysPtrIdx, uint8 * HwPtrIdx)
{
    Canexcel_Ip_DescState retval;
    uint32 DCSTA = base->DSCMBCTRLAR[descNo].DCSTA;

    switch (DCSTA & CANXL_DSC_CONTROL_DCSTA_STATE_MASK)
    {
        case 0u:
            retval = CANEXCEL_DESC_STATE_INACTIVE;
            break;
    ...
    if (NULL_PTR != SysPtrIdx)
    {
        *SysPtrIdx = (uint8)((DCSTA & CANXL_DSC_CONTROL_DCSTA_SYSPOINTER_MASK) >> CANXL_DSC_CONTROL_DCSTA_SYSPOINTER_SHIFT);
    }

    if (NULL_PTR != HwPtrIdx)
    {
        *HwPtrIdx = (uint8)((DCSTA & CANXL_DSC_CONTROL_DCSTA_HWPOINTER_MASK) >> CANXL_DSC_CONTROL_DCSTA_HWPOINTER_SHIFT);
    }
 ...
 */
void CanXL_ConfigAccAddr(CANXL_RXFIFO_Type * base,const Canexcel_Ip_RxFifoFilterADDR * filter, uint8 filtIdx);
 /*!
 ...
 * @param   filtIdx  Filter Position
 */
void CanXL_ConfigIDFilter(CANXL_RXFIFO_Type * base,const Canexcel_Ip_RxFifoFilterID * filter, uint8 filtIdx);
 /*!
...
void Canexcel_Ip_ErrIRQHandler(uint8 instance);
 ...
 /*!
 * @brief Spurious IrqHandler for MRU
 *
 * @param   instance The CanXL Instance Number
 */
