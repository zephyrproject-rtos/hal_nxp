...
#define CANXL_IP_TX_TIMESTAMPL_INIT_VALUE   (0x00000000U)
#define CANXL_IP_TX_TIMESTAMPH_INIT_VALUE   (0x00000001U)
#define CANEXCEL_IP_MAX_SIZEOF_FIFO_OBJ_HEADER ((uint16)RX_XL_DATA + (3u*sizeof(uint32))) /*header + AF + timestamp + 1 word(in case payload length is not multiple of word)*/
/*==================================================================================================
...
#define CAN_43_CANEXCEL_START_SEC_VAR_CLEARED_UNSPECIFIED_NO_CACHEABLE
#include "Can_43_CANEXCEL_MemMap.h"
CANEXCEL_IP_STATE_EXT
#define CAN_43_CANEXCEL_STOP_SEC_VAR_CLEARED_UNSPECIFIED_NO_CACHEABLE
#include "Can_43_CANEXCEL_MemMap.h"
/*==================================================================================================
*                                    FUNCTION PROTOTYPES
==================================================================================================*/
 ...
 *  @details   This function will config transmit parameters from Canexcel_Ip_TxDataInfoType and
 *             the message Id
...
/**
 *  @brief     CanEXCEL config FD Message for Tx with Id, length
 *  @details   This function will config transmit parameters from Canexcel_Ip_TxDataInfoType and
 *             the message Id
 *  @param[in] id        ID of the message to transmit
 *  @param[in] info      Data info
 ...
 *  @details   This function will config receive parameters from Canexcel_Ip_RxMdConfigType and
 ...
 *
 *  Note: The driver does not ensure synchronization between different threads trying
 *  to push a buffer at the same time. This synchronization shall be implemented by the
 *  application. The buffer should be reused only when it was completed transfer(check buffer's address returned by Canexcel_Ip_GetRxBuffStatus).
 *
 *  @param[in] instance A CanEXCEL instance number
 *  @param[in] descNo   Index of the message descriptor
 *  @param[in] rxPtrList Pointer to the message buffer
 *  @return    CANEXCEL_STATUS_SUCCESS if successful;<br>
 *             CANEXCEL_STATUS_BUSY if all buffers were fully pushed into the descriptor but no ones processed.<br>
 *             CANEXCEL_STATUS_ERROR if the descriptor is not configured by Canexcel_Ip_ConfigRx.
 */
Canexcel_Ip_StatusType Canexcel_Ip_RxDescriptor(uint8 instance, uint8 descNo, uint32 rxPtrList);


/**
 *  @brief     Pop received data from the RxFIFO.
 *  @details
 *
 *  @param[in] instance A CanEXCEL instance number
 *  @return    CANEXCEL_STATUS_SUCCESS if operation successful
 *             CANEXCEL_STATUS_NO_DATA_AVAILABLE if there is no data pushed into RX FIFO
 *             CANEXCEL_STATUS_ERROR if the RXFIFO is not activated on CanEXCEL instance.
 */
 ...
 *  Note: The driver does not ensure synchronization between different threads trying
 *  to push a buffer at the same time. This synchronization shall be implemented by the
 *  application. The buffer should be reused only when it was completed transfer(check buffer's address returned by Canexcel_Ip_GetTxBuffStatus).
 *
 *  @param[in] instance A CanEXCEL instance number
 *  @param[in] descNo   Index of the message descriptor
 ...
 *  @return    CANEXCEL_STATUS_SUCCESS if successful;<br>
 *             CANEXCEL_STATUS_BUSY if all buffers were fully pushed into the descriptor but no ones processed.<br>
 *             CANEXCEL_STATUS_ERROR if the buffer txPtrList is not configured(Canexcel_Ip_ConfigFdTx/Canexcel_Ip_ConfigXlTx).
 */
 ...
 *  @param[in] instance  A CanEXCEL instance number
 *  @param[in] descNo    Index of the message descriptor
 *  @param[in] RxMsg     The CanEXCEL receive message buffer pointer.
 *  @return    CANEXCEL_STATUS_SUCCESS if successful;<br>
 *             CANEXCEL_STATUS_BUSY if all buffers were fully pushed into the descriptor but no ones processed.<br>
 *             CANEXCEL_STATUS_ERROR if the descriptor is not configured by Canexcel_Ip_ConfigRx.
 */
...
/**
 *  @brief     Get number of buffers completed tranfser.
 *  @details   This function returns the total number of buffers completed transfer, the number will be decreased
 *             when a frame read by Canexcel_Ip_GetRxBuffStatus/Canexcel_Ip_GetTxBuffStatus
 *
 *  Note:      For Rx buffer, completed transfer means that a frame was successfully received by the buffer.
 *             For Tx buffer, completed transfer means that a frame was successfully transmitted or aborted due to retransmission counter exhaust by the buffer.
 *
 *
 *  @param[in] u8Instance A CanEXCEL instance number
 *  @param[in] u8DescNo   Index of the message descriptor
...
/**
 *  @brief     Read a frame received by the Rx Message Descriptor.
 *  @details   This function reads the first received frame in the Rx Message Descriptor.
 *             The address of buffer containing the frame pushed by calling of Canexcel_Ip_ReceiveFD/Canexcel_Ip_ReceiveXL will be updated as parameter if required
 *             to indicate that the buffer was completed receiving.
 *
 *
 * Note:       Once the application finished processing the buffer, it could be reused for further receptions
 *             by Canexcel_Ip_ReceiveFD/Canexcel_Ip_ReceiveXL.
 *
 *
 *  @param[in]  u8Instance A CanEXCEL instance number
 *  @param[in]  u8DescNo Index of the message descriptor
 *  @param[out] pRxPtrList The address of buffer containing the frame. If this information is not needed, this parameter should be NULL_PTR.
 *  @param[out] pRxBuffInfor Information related to the data contained by this receive buffer. If this information is not needed, this parameter should be NULL_PTR.
 *  @param[out] pData Pointer to payload of frame. If this information is not needed, this parameter should be NULL_PTR.
 *  @return    CANEXCEL_STATUS_SUCCESS if a frame was successfully read and no more frames are available.<br>
 *             CANEXCEL_STATUS_MORE_DATA_AVAILABLE if a frame was successfully read and at least one more frame is available.<br>
 *             CANEXCEL_STATUS_NO_DATA_AVAILABLE if no frames are available.
 */
...
/**
 *  @brief     Read a frame completed transfer by the Tx Message Descriptor.
 *  @details   This function reads the first completed transfer frame in the Tx Message Descriptor.
 *             The address of buffer pushed by calling of Canexcel_Ip_SendFDMsg/Canexcel_Ip_SendXLMsg will be updated as parameter if required
 *             to indicate that the buffer was completed transfer(transmitted successfully or aborted due to retransmission counter exhaust).
 *
 *
 * Note:       Once the application finished processing the buffer, it could be reused for further transmissions
 *             by Canexcel_Ip_SendFDMsg/Canexcel_Ip_SendXLMsg.
 *
 *
 *  @param[in]  u8Instance A CanEXCEL instance number
 *  @param[in]  u8DescNo Index of the message descriptor
 *  @param[out] pTxPtrList The address of buffer containing the frame. If this information is not needed, this parameter should be NULL_PTR.
 *  @param[out] pTxBuffInfor Information related to the data contained by this Tx buffer.
 *              The frame was successfully transmitted if pTxBuffInfor->NoError is TRUE else it was aborted due to retransmission counter exhaust.
...
/**
 *  @brief     Read a frame received by the Rx FIFO.
 *  @details   This function reads the first received frame in the Rx FIFO.
 *
 *  @param[in]  u8Instance A CanEXCEL instance number
 *  @param[out] pRxPtrList The address of buffer containing the frame. If this information is not needed, this parameter should be NULL_PTR.
 *  @param[out] pRxBuffInfor Information related to the data contained by this receive buffer. If this information is not needed, this parameter should be NULL_PTR.
 *  @param[in]  checkForSpecificFrameType Indicates if only a specific frame type must be checked for in the FIFO.
 *  @return    CANEXCEL_STATUS_SUCCESS if a frame was successfully read and no more frames are available.<br>
 *             CANEXCEL_STATUS_MORE_DATA_AVAILABLE if a frame was successfully read and at least one more frame is available.<br>
 *             CANEXCEL_STATUS_NO_DATA_AVAILABLE if no frames are available.
 */
...
/**
 *  @brief     Get MD RX buffer underrun Status
 *  @details   Check if a MD  rx buffer unerrun error was reported by CANEXCEL controller instance.
 *  @param[in] instance A CanEXCEL instance number
 *  @return    True CMDUERR set
 *             False CMDUERR cleared
 */
boolean Canexcel_Ip_GetMdUnderrunError(uint8 instance);

/**
 *  @brief     Get RXFIFO buffer overrun Status
 *  @details   Check if a RXFIFO buffer overrun error was reported by CANEXCEL.
 *  @param[in] instance A CanEXCEL instance number
 *  @return    True RXFEF set
 *             False RXFEF cleared
 */
boolean Canexcel_Ip_GetRxFifoOverrunError(uint8 instance);

/**
 *  @brief     Get RX SMB Overrun status
 *  @details   Check if a the internal two Rx SMB (SMB0 or SMB1) are ovewritten by a received frame
 *  @param[in] instance A CanEXCEL instance number
 *  @return    True CRXOERR set
 *             False CRXOERR cleared
 */
 ...
 * @param[in]   maxNoOfMbDesc        Max number of descriptors of the controller
 */
...
/**
 * @brief   This function gets the Enable\Disable Interrput status for a specific Message Descriptor
 * @param[in]       Instance         A CanExcel instance number
 * @param[in]       descNo           A descriptor Number for which to return the interrupt activation status
 */
...
/**
 * @brief De-activates the RXFIFO for the given controller instance.
 * @details This function shall de-activate RXFIFO for the given controller instance and it can't participate in the message  Rx process.
 * @note    The system pointer and hardware pointer of the RXFIFO equals to zero after de-activation.
 *
 * @param[in]   instance         A CanExcel instance number
 * @return    CANEXCEL_STATUS_SUCCESS if successful;<br>
 *            CANEXCEL_STATUS_TIMEOUT if fail to set;<br>
