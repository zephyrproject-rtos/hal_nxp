...
/* Check if current file and CanEXCEL_Ip_DevReg.h are of the same vendor */
#if (CANEXCEL_IP_TYPES_VENDOR_ID_H != CANEXCEL_IP_DEVREG_VENDOR_ID_H)
    #error "CanEXCEL_Ip_Types.h and CanEXCEL_Ip_DevReg.h have different vendor ids"
#endif
/* Check if current file and CanEXCEL_Ip_DevReg.h are of the same Autosar version */
#if ((CANEXCEL_IP_TYPES_AR_RELEASE_MAJOR_VERSION_H    != CANEXCEL_IP_DEVREG_AR_RELEASE_MAJOR_VERSION_H) || \
     (CANEXCEL_IP_TYPES_AR_RELEASE_MINOR_VERSION_H    != CANEXCEL_IP_DEVREG_AR_RELEASE_MINOR_VERSION_H) || \
     (CANEXCEL_IP_TYPES_AR_RELEASE_REVISION_VERSION_H != CANEXCEL_IP_DEVREG_AR_RELEASE_REVISION_VERSION_H) \
)
    #error "AutoSar Version Numbers of CanEXCEL_Ip_Types.h and CanEXCEL_Ip_DevReg.h are different"
#endif
/* Check if current file and CanEXCEL_Ip_DevReg.h are of the same Software version */
#if ((CANEXCEL_IP_TYPES_SW_MAJOR_VERSION_H != CANEXCEL_IP_DEVREG_SW_MAJOR_VERSION_H) || \
     (CANEXCEL_IP_TYPES_SW_MINOR_VERSION_H != CANEXCEL_IP_DEVREG_SW_MINOR_VERSION_H) || \
     (CANEXCEL_IP_TYPES_SW_PATCH_VERSION_H != CANEXCEL_IP_DEVREG_SW_PATCH_VERSION_H) \
)
    #error "Software Version Numbers of CanEXCEL_Ip_Types.h and CanEXCEL_Ip_DevReg.h are different"
#endif
/*==================================================================================================
*                                      DEFINES AND MACROS
==================================================================================================*/
...
typedef struct {
    CANXL_SIC_Type ** SIC;
    CANXL_MRU_Type ** MRU;
    CANXL_GRP_CONTROL_Type ** GRP;
    CANXL_DSC_CONTROL_Type ** DESC_CTR;
    CANXL_MSG_DESCRIPTORS_Type ** MSGD;
    CANXL_RXFIFO_CONTROL_Type ** RXF_CNT;
    CANXL_RXFIFO_Type ** RXFIFO;
    CANXL_FILTER_BANK_Type ** FILTER;
...
typedef enum
{
    ...
    ,CANEXCEL_CLASIC_FD_FRAME /**< Accept both Classic and FD frames. Used only for Rx Descriptor configuration */
#endif
...
/** @brief The status of RX FIFO state.
 *  @details The RX FIFO control Status mechanism
 */
/* implements Canexcel_Ip_RxFifoState_typedef */
typedef enum
{
    CANEXCEL_FIFO_STATE_INACTIVE,
    CANEXCEL_FIFO_STATE_EMPTY,
    CANEXCEL_FIFO_STATE_NOTFULL,
    CANEXCEL_FIFO_STATE_FULL,
    CANEXCEL_FIFO_STATE_OVERRUN
} Canexcel_Ip_RxFifoState;
...
typedef enum
{
    ...
    CANEXCEL_IP_INT_MRUIE           /*!< MRU event interrupt*/
 ...
 * Please check the platform RM for the definitions of time sources.*/
/* implements  Canexcel_Ip_TimeBaseSelType_enum */
typedef enum
{
    CANTBS_TIMESURCE_BUS0,
    CANTBS_TIMESURCE_BUS1,
    CANTBS_TIMESURCE_BUS2,
    CANTBS_TIMESURCE_BUS3
...
typedef enum
{
    ...
    CANEXCEL_STATUS_MORE_DATA_AVAILABLE,      /**< A buffer completed transfer and at least another one in the MD detected */
    CANEXCEL_STATUS_NO_DATA_AVAILABLE         /**< No buffers completed transfer */
...
/* implements  Canexcel_Ip_RxFifoFilterID_typedef */
typedef struct{
    uint8 ideBitValue;
    Canexcel_Ip_RxFifoFilterType filterType;
    uint32 idFilterH;    /* Filter ID element in case of mask filter type this is ID value, in case of range filter type this is the higher value. */
    uint32 idFilterL;    /* Filter ID element in case of mask filter type this is mask value, in case of range filter type this is the lower value. */
} Canexcel_Ip_RxFifoFilterID;
/* implements  Canexcel_Ip_RxFifoFilterADDR_typedef */
typedef struct{
    Canexcel_Ip_RxFifoFilterType filterType;
    uint32 addrFilterH;    /* Filter ADDR element in case of mask filter type this is ADDR value, in case of range filter type this is the higher value. */
    uint32 addrFilterL;    /* Filter ADDR element in case of mask filter type this is mask value, in case of range filter type this is the lower value. */
} Canexcel_Ip_RxFifoFilterADDR;
...
typedef struct{
    ...
    uint8  noSduFilters;
    uint8  noVcanFilters;
    const Canexcel_Ip_RxFifoFilterID * IdFilterPtr;   /* Pointer to the array with ID filter configs */
    const Canexcel_Ip_RxFifoFilterADDR * AddrFilterPtr; /* Pointer to the array with Acceptance Address filter configs */
...
typedef struct {
    /** @brief Vcan of a CAN XL message. */
    uint16 VCID;
    ...
    boolean SEC;
} Canexcel_Ip_XLParamsType;

/* implements  Canexcel_Ip_TxDataInfoType_structure */
typedef struct{
    ...
    Canexcel_Ip_XLParamsType * XlParamsPtr;      /**< XL parameters, must not be a NULL_PTR if frame is XL_FRAME else should be a NULL_PTR   */
    boolean is_polling;
} Canexcel_Ip_TxDataInfoType;

/* implements  Canexcel_Ip_RxDataInfoType_structure */
typedef struct{
    Canexcel_Ip_FrameType frame;
    Canexcel_Ip_MsgBuffIdType idType;
    uint32 MessageId;
    boolean enable_brs;                    /**< Enable bit rate switch inside a CAN FD format frame*/
    uint16 dataLength;                     /**< Length of data in bytes */
    Canexcel_Ip_XLParamsType * XlParamsPtr;  /**< XL parameters, should not be a NULL_PTR if frame is XL_FRAME else should be a NULL_PTR   */
    uint32 timeStampL;
    uint32 timeStampH;
} Canexcel_Ip_RxDataInfoType;

/* implements  Canexcel_Ip_RxMdXlFilterType_structure */
typedef struct {
    uint8 FilterBankSel;      /**< Filter Bank selection: 0 or 1 */
    boolean VcanRoa;          /**< VCAN Rejection Over Acceptance: FALSE - Acceptance over rejection, TRUE - Rejection over acceptance */
    boolean SduRoa;           /**< SDU Rejection Over Acceptance:  FALSE - Acceptance over rejection, TRUE - Rejection over acceptance */
    boolean AddrRoa;          /**< ADDR Rejection Over Acceptance: FALSE - Acceptance over rejection, TRUE - Rejection over acceptance */
    uint8 AccptVcanPos;       /**< Defines the VCAN filter number of the selected filter bank (FilterBankSel) to be used for VCAN acceptance filtering criteria. */
    uint8 AccptSduPos;        /**< Defines the SDU filter number of the selected filter bank (FilterBankSel) to be used for SDU acceptance filtering criteria. */
    uint8 AccptAddrPos;       /**< Defines the ADDR filter number of the selected filter bank (FilterBankSel) to be used for ADDR acceptance filtering criteria. */
    uint8 RejVcanPos;         /**< Defines the VCAN filter number of the selected filter bank (FilterBankSel) to be used for VCAN rejection filtering criteria. */
    uint8 RejSduPos;          /**< Defines the SDU filter number of the selected filter bank (FilterBankSel) to be used for SDU rejection filtering criteria. */
    uint8 RejAddrPos;         /**< Defines the ADDR filter number of the selected filter bank (FilterBankSel) to be used for ADDR rejection filtering criteria. */
    boolean Sec;              /**< Defines the SEC value to be compared with the received message SEC for acceptance in this message descriptor. */
    boolean SecMask;          /**< Defines the SEC mask, which is used to filter the received SEC field for the message acceptance criteria. */
} Canexcel_Ip_RxMdXlFilterType;

/* implements  Canexcel_Ip_RxMdConfigType_structure */
typedef struct{
    Canexcel_Ip_FrameType frame;
    Canexcel_Ip_MsgBuffIdType idType;
    boolean IDEMask;                                /**< IDE filter mask */
    boolean RTRRRS;                                 /**< RTR filter value for classic CAN, RRS filter value for CANFD/CANXL frame */
    boolean RTRRRSMask;                             /**< RTR filter mask for classic CAN, RRS filter mask for CANFD/CANXL frame */
    Canexcel_Ip_RxFifoFilterType IdFilterType;      /**< Selects mask or range filtering acceptance criteria for ID field */
    uint32 IdL;                                     /**< When CANEXCEL_IP_RX_FIFO_RANGE_FILTER selected(IdFilterType) then this field is range low else is filter mask  */
    uint32 IdH;                                     /**< When CANEXCEL_IP_RX_FIFO_MASK_FILTER selected(IdFilterType) then this field is range high else is filter value  */
    uint16 MbSize;                                  /**< Length of data in bytes */
    Canexcel_Ip_RxMdXlFilterType * XlParamsFilterPtr;  /**< XL Params filter configuration */
} Canexcel_Ip_RxMdConfigType;
...
enum
{
  TX_FD_DATA = TX_HEADER_END
};

enum
{
    TX_XL_AF = TX_HEADER_END,
    TX_XL_DATA
};
...
enum
{
    RX_FD_DATA = RX_HEADER_END,
    RX_FD_TIMESTAMPL = RX_FD_DATA + (64 / sizeof(uint32)),
    RX_FD_TIMESTAMPH
};

enum
{
    RX_XL_AF = RX_HEADER_END,
    RX_XL_DATA,
    RX_XL_TIMESTAMPL = RX_XL_DATA + (2048 / sizeof(uint32)),
    RX_XL_TIMESTAMPH
};
...
typedef struct
{
    ...
    uint8   currBufferIdx;             /**< The index of buffer completed transfer */
...
/*! @brief Information needed for internal handling of a given RXFIFO. */
/* implements  Canexcel_Ip_RXFIFOhandleType_struct */
typedef struct
{
    uint8   currBufferIdx;             /**< The index of buffer completed transfer */
    boolean isPolling;                 /**< True if the transfer is Polling Mode  */
    boolean isXLFrame;                 /**< True for XLFrame Types, False for Classic and FD frames. */
} Canexcel_Ip_RXFIFOhandleType;


/* implements  Canexcel_Ip_TxBuffStatusType_structure */
typedef struct
{
    boolean NoError;    /**< TRUE if the frame was successfully transmitted else it was aborted due to retransmission counter exhaust. */
    uint32 timeStampL;
    uint32 timeStampH;
} Canexcel_Ip_TxBuffStatusType;
...
{
   Canexcel_Ip_MDhandleType * msgDescListPtr;
   Canexcel_Ip_RXFIFOhandleType rxFifo;
...
typedef struct{
    ...
    Canexcel_Ip_MDhandleType * allocatedMsgDescList;/**< Pointer to the array of structures that is generated and allocated. */
