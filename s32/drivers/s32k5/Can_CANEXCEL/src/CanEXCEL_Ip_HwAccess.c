...
#define CANEXCEL_IP_HWACCESS_AR_RELEASE_MINOR_VERSION_C       9
#define CANEXCEL_IP_HWACCESS_AR_RELEASE_REVISION_VERSION_C    0
#define CANEXCEL_IP_HWACCESS_SW_MAJOR_VERSION_C               0
#define CANEXCEL_IP_HWACCESS_SW_MINOR_VERSION_C               8
...
{
   uint16 idx;
   CANXL_MSG_DESCRIPTORS_Type * base_Msg_Desc = CANXL->MSGD[instance];
   ...
   volatile uint32 * ptr = (volatile uint32 *)&CANXL->FILTER[instance]->AFCFG0;
   for (idx = 0u; idx <= (uint16)(CANXL_IP_FILTER_BANK_SIZE/4U); idx++)
   {
       ptr[idx] = 0U;
   }
   /* Clear and initilize RXFIFO memory */
   CANXL_RXFIFO_Type * base_RxFifo = CANXL->RXFIFO[instance];
...
{
    uint32 * Mb_Data_32 = MB;
    uint8 * Mb_Data_8 = (uint8 *)MB;
...
void CanXL_ConfigAccAddr(CANXL_RXFIFO_Type * base,const Canexcel_Ip_RxFifoFilterADDR * filter, uint8 filtIdx)
{
    ...
    base->ADDRACPTFLTAR[filtIdx].AAFLTH = filter->addrFilterH;
    base->ADDRACPTFLTAR[filtIdx].AAFLTL = filter->addrFilterL;
}
/*FUNCTION**********************************************************************
 *
...
void CanXL_ConfigIDFilter(CANXL_RXFIFO_Type * base,const Canexcel_Ip_RxFifoFilterID * filter, uint8 filtIdx)
{
    ...
    base->IDACPTFLTAR[filtIdx].IDAFLTH = ((filter->ideBitValue == 1u)?filter->idFilterH : CANXL_RXFIFO_IDAFLTH_IDSTANDn_H(filter->idFilterH)) | CANXL_RXFIFO_IDAFLTH_IDEn_H(filter->ideBitValue);
    base->IDACPTFLTAR[filtIdx].IDAFLTL = ((filter->ideBitValue == 1u)?filter->idFilterL : CANXL_RXFIFO_IDAFLTL_IDSTANDn_L(filter->idFilterL)) | CANXL_RXFIFO_IDAFLTL_IDEn_L(1u);
}
/*FUNCTION**********************************************************************
 *
